...
app
  ...
  .get('/api/user/getPublicKeyFingerprint', (req, res) => {
    console.log("Serving fingerprint request");
    res.json({
      success: true,
      data: {
  ...
  .get('/captcha', (req, res) => {
    console.log("Serving captcha request");
    res.json({
      success: true,
      code: 200,
      message: 'success',
  ...
  // Then proxy other API requests
  .use('/api', createProxyMiddleware(config.proxy))
  // Finally, the history middleware and static file serving
  .use(history())
  .use(
    express.static('./dist', {
